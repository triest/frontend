...
class GirlsController extends Controller
{
    ...
    public function showGirl($id)
    {
        ...
        $girl = Girl::select(['name', 'email', 'password','id','phone','description','enabled','payday','payed','login','main_image','sex','meet','weight','height','age'])->where('id', $id)->first();
    //    dump($girl);
        if($girl==null){
            return  $this->index();
        }
     ...
     //   dump($images);
        ...
        return view('girlView')->with(['girl'=>$girl,'images'=>$images]);
...
//dump($girl);
  ...
  //          dump($user);
    ...
    public function reciverYandex(Request $request){
     ...
     //   echo $user_email;
        $user_money=$request['withdraw_amount'];
        ...
        if($operation_id=='test-notification'){
            try {
                DB::insert('INSERT INTO `money_history`(`date`, `user_email`, `received`,`operation_id`) VALUES (?,?,?,?)', [$date,'no', $user_money,'test']);
            }
...
//            echo '<br>';
     ...
     //           dump($end_vip);
         ...
         //   die();
            //теперь списываем деньги
            $new_money=$have_user->money-$price;
           // dump($new_money);
        //    echo 'new price '; echo $new_money;
            DB::table('users')->where('id',$id)->update(['money'=>$new_money]);
            //обновляем анкету
...
/*
    ...
    public function girlsEditAuchAnket(){
      //  dump($girl_id);
        ...
        $girl = Girl::select(['name','age', 'email', 'password','id','phone','description','enabled','payday','payed','login','main_image','sex','meet','weight','height'])->where('user_id', $user->id)->first();
  //   dump($girl);
      ...
      //  dump($images);
        ...
        return view('editGirl')->with(['girl'=>$girl,'phone'=>$phone]);
    }
